...
export default function Tasks() {
  ...
  const getCardColor = (priority, completed) => {
  ...
  const cardStyles = (priority, completed, isFading) => ({
    marginBottom: "20px",
    padding: "15px",
    borderRadius: "12px",
  ...
  const pastelButtonColor = (lightColor, darkColor) => ({
  ...
  const sortTasks = (taskList) => {
    ...
    switch (sortBy) {
      case "priority":
        ...
        return sortedTasks.sort((a, b) => (priorityOrder[a.priority] || 4) - (priorityOrder[b.priority] || 4));
      case "dueDate":
        return sortedTasks.sort((a, b) => new Date(a.dueDate || Infinity) - new Date(b.dueDate || Infinity));
      case "lastAdded":
          ...
          {sortTasks(tasks)
            ...
            .map((task) => {
              ...
              return (
                <li
                  key={task.id}
                  style={cardStyles(task.priority, task.completed, fadingTasks.includes(task.id))}
                >
